...
function fillDocumentsForRow(row) {
  ...
  if (date < currDate) {
    console.log(`[DATE] Исходная дата ${date} меньше текущей, заменяем на ${currDate}`);
    date = currDate;
  }
  ...
  var agrNum = genAgrNum1(row) + ' от ' + agrDate;
  var agrNumTS = genAgrNum1(row);
